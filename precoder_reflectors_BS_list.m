function precoder_reflectors_BS_list(varargin)
...
if isempty(varargin)
    ...
    par.BS          = 128000;       % Antanna of BaseStation
    par.BS_list     = [32:32:256];
    par.UE          = 4;       % Number of User Equipment
    par.order       = 4;        % PSK modulation order
    par.P           = 1;       % Number of reflectors
    par.trials      = 500;      % number of Monte-Carlo trials (transmissions)
    par.SNRdB_list  = -10:1:10;    % list of SNR [dB] values to be simulated
    par.precoder    = {'CI'};%'GD','AO-GD','AO-CVX','CI','CI-R'
...
% -- initialization
par.simName = ['BER_',num2str(par.UE),'Users_',num2str(par.BS),'antennas_and',num2str(par.P),'reflectors'...
...
res.SER = zeros(length(par.precoder),length(par.SNRdB_list),length(par.BS_list));
...
time_elapsed = 0; tic;
for jjj = 1:length(par.BS_list)
    par.BS = par.BS_list(jjj);
    for tt=1:par.trials
        ...
        G  = sqrt(0.5)*(randn(par.P ,par.BS)+1i*randn(par.P ,par.BS));
        for pp=1:length(par.precoder)
            ...
            for k=1:length(par.SNRdB_list)
                ...
                res.SER(pp,k,jjj) = res.SER(pp,k,jjj) + sum(err)/par.UE/par.rep_time; % symbol error rate
    ...
    if toc>10
        ...
        fprintf('estimated remaining simulation time: %3.0f min.\n',time_elapsed*(length(par.BS_list)/jjj-1)/60);
        tic;
