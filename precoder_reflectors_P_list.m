function precoder_reflectors_P_list(varargin)
...
res.SER = zeros(length(par.precoder),length(par.SNRdB_list),length(par.P_list));
...
time_elapsed = 0; tic;
for jjj = 1:length(par.P_list)
    par.P = par.P_list(jjj);
    for tt=1:par.trials
    ...
    if toc>10
        ...
        fprintf('estimated remaining simulation time: %3.0f min.\n',time_elapsed*(length(par.P_list)/jjj-1)/60);
        tic;
